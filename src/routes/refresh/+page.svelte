<script lang="ts">
  ...
  import { uploadFile, supabase } from "$lib/clients/supabase";
  ...
  let redirectTo = "https://localhost:5173/refresh";

  export let data: { session: SupaSession };
  let { session: supaSession } = data;
  ...
  async function onRefresh() {
    ...
    await uploadFile(data, "reading_list");
  }
</script>
...
<main>
  {#if supaSession}
    <div>
      Supa session: [{supaSession.user.id} / {supaSession.user.email} / {supaSession
        .user.user_metadata.email_verified}]
    </div>
  {:else}
    <div class="supa row flex-center flex">
...
<style lang="scss">
  .supa {
    // quickstart CSS for supabase auth form (TODO: simplify, switch to pico)
